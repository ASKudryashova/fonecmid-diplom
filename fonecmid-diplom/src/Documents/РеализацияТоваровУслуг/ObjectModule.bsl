...
//ВКМ Кудряшова А.С. Установка проверки заполнения реквизита "Основание"
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Данные = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "ВидДоговора");
	
	Если Данные.ВидДоговора <> АбонентскийДоговор И Не ЗначениеЗаполнено(Основание) Тогда
	
		ТекстСообщения = СтрШаблон("Для договора вида ""%1"" должно быть заполнено основание, которым является документ Заказ покупателя.", 
			Данные.ВидДоговора);
...
//ВКМ Кудряшова А.С. добавление автозаполнение таблицы услуги
Процедура ВКМ_ВыполнитьАвтозаполнение() Экспорт  
	...
	Если Не ЗначениеЗаполнено(АбонентскаяПлата) Тогда
		ОбщегоНазначения.СообщитьПользователю("Не указана сумма абонентской платы");
		Возврат;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(РаботыСпециалиста) Тогда
		ОбщегоНазначения.СообщитьПользователю("Не указаны работы специалиста");
		Возврат;
	КонецЕсли;
